...
class consultas {
	...
	public function consultar_perfiles($perfil_id){
		...
		$proveedor->connect();
		$sql=" 
			select * from perfiles where perfil_status<>'E'
		";
	...
	public function guardar_perfil($perfil_id, $perfil_nombre, $perfil_modifico, $perfil_status){
		...
		$sql_repetidos=" select * from perfiles where rtrim(ltrim(perfil_nombre))='".$perfil_nombre."' and perfil_status<>'E' ";
		...
		if (!$repetidos->error()){
			if ($repetidos->rows($repetidos->stmt) == 0) {
				...
				$proveedor_M->connect();
				//Obtengo el Id Maximo
				$valormaximo=" select CASE when max(perfil_id+1) IS NULL then 1 else (max(perfil_id + 1)) end as IndiceMaximo from perfiles ";
